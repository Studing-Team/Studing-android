...
class HomeScrapNoticeListAdapter(
    private var activity: MainActivity,
    private var scrapNotices: List<ScrapNotice>
) :
    RecyclerView.Adapter<HomeScrapNoticeListAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.writerType.run {
            text = splitString(scrapNotices[position].affiliation.toString())?.first
            when (splitString(scrapNotices[position].affiliation.toString())?.second) {
                "총학생회" -> {
